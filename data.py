...
def get_merged_data_with_em():
    ...
    # ✅ Make sure 'Supalearn Password' stays in the merged data
    columns_to_keep = list(main_data.columns)  # keep all columns including Supalearn Password

    merged_data = main_data.merge(em_data[['Student ID', 'EM', 'Phone Number']], on="Student ID", how="left")

    merged_data = merged_data[columns_to_keep + ['EM', 'Phone Number']]  # retain Supalearn Password and EM info
